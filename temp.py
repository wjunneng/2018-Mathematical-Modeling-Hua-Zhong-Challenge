...
#    readmitted = {'NO':0, '<30': 1, '>30':1}
    data[u'再次入院'].replace(['NO', '<30', '>30'], [0, 1, 1], inplace=True)
...
def question_three(path):
    ...
    for i in range(len(attribute)):
        ...
        for j in group_labels:
            number.append(data[data[attribute[i]] == j].shape[0])
...
#    attribute_data = data[[u'药物类型是否改变', u'是否服用药物', u'再次入院']]    
#    corMat = pd.DataFrame(attribute_data.corr())
#    # 输出相关系数
#    print(corMat)
#    # 绘制皮尔逊相关系数热图
#    plt.pcolor(corMat)
...
# attribute = [u'诊断号', u'病人号', u'种族',	 u'性别', u'年龄', u'体重', u'入院类型', u'出院去处',	u'入院来源',
#              u'入院天数', u'诊疗医师专业', u'药物治疗次数', u'门诊次数', u'急诊次数', u'住院次数',
#              u'诊断1', u'诊断2', u'诊断3', u'诊断次数', u'葡萄糖血清检测', u'HbA1C检测', u'糖尿病类型改变',
#              u'糖尿病患者', u'再次入院']

# attribute = [u'二甲双胍', u'瑞格列奈', u'那格列胺', u'氯磺丙脲', u'格列美脲', u'乙酰苯磺酰环己脲', u'格列甲嗪',
#              u'优降糖', u'甲糖宁', u'匹格列酮', u'罗格列酮',u'阿卡波糖', u'米格列醇', u'曲格列酮', u'甲磺吖庚脲',
#              u'甲酰胺', u'西他列汀', u'胰岛素', u'优降糖，二甲双胍', u'格列甲嗪，二甲双胍', u'格列美脲，匹格列酮',
#              u' 二甲双胍，罗格列酮', u'二甲双胍，匹格列酮']    
#    # 筛选不是患者的病人
#    patient = data[data[u'糖尿病患者'] == 0]
#    # 筛选不是患者的挂号记录
#    patient_log = data[data[u'病人号'].isin(list(patient[u'病人号']))]    
#    # 以诊断号为index
#    patient_log.index = list(patient_log[u'诊断号'])[0:]
#    # 删除诊断号列
#    del patient_log[u'诊断号']
#    # 根据诊断号进行排序
#    patient_log.sort_index(inplace=True)
#    
#    # 前期不是患者
#    one = 0
#    # 前期是患者
...
#    
#    # 剔除无用的记录
#    for name, group in patient_log.groupby(u'病人号'):
#        if list(group[u'糖尿病患者'].unique()) == list([0]):            
#            three += 1
#        else:
#            if group.at[group.index[0], u'糖尿病患者'] == 0:
#                group = group[group[u'糖尿病患者'] == 1]
...
#    
#    for name, group in result.groupby(u'病人号'):
#        first_weights = 1/group.shape[0]
#        for i in group.index:
#    
#    result.to_csv('数据\question_1.csv')    